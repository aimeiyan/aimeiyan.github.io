...
class MyHandler(tornado.web.RequestHandler):
    ...
    def get_current_user(self):
        if MyHandler.session_cookie_key in self.cookies:
            ...
            return MyHandler.session.get(session_id.value, {})
        return {}
    ...
    def set_user_data(self, key, value):
        ...
        if session_id not in MyHandler.session:
            MyHandler.session[session_id] = {}

        MyHandler.session[session_id][key] = value
        if key == 'userid':
            self.set_user_id()
...
class BlogHandler(MyHandler):
    def get(self, blog_id):
        ...
        if len(blogs):
            ...
            self.render("blog-detail.html", blog_detail_id=data['id'], blog_detail_title=data['title'], blog_detail_create_time=data['create_time'],
                        blog_detail_article=data['article'],login_email_address=self.email_address, comments=comments, lists=lists)
        else:
            self.set_status(404)
    ...
    def post(self, *args, **kwargs):
        blogid = self.get_argument('blogid', u'')
        ...
        self.redirect('%s#comment-%d' % (self.request.path, comment_id))
